...
type Colum = ComponentFramework.PropertyHelper.DataSetApi.Column;
...
(dataset: DataSet, columnTotalWidth: number, tableWidth: number, column: Colum) => {
  console.log(column);
  if (column.name === 'meo_tomid') {
    console.log(column.visualSizeFactor);
    return 90;
  }
  if (column.name === 'meo_levelofcompliancecode') {
    console.log('Complicance');
    return 160;
  }
...
(dataset: DataSet, _service: IDataverseService): IColumn[] => {
  ...
  //console.log(columnTotalWidth, dataset.columns);
  return dataset.columns
    ...
    .map<IColumn>((column): IColumn => ({
      ...
      calculatedWidth: column.visualSizeFactor +
        calculateAdditinalWidth(dataset, columnTotalWidth, tableWidth, column),
