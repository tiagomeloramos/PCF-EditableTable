...
export class DataverseService implements IDataverseService {
  ...
  constructor(context: ComponentFramework.Context<IInputs>) {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    this._portalUrl = `${this._context.page.getClientUrl()}/_api/`;
  ...
  public async getFieldSchemaName(): Promise<string> {
    return 'meo_ropaid';
    // do something here to save this entity definitions on the assist table
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
  ...
  public async getReqirementLevel(fieldName: string) {
    // eslint-disable-next-line max-len
    const request = `/_api/meo_entitydefinitionses?$select=meo_definitionvalue&$filter=(meo_entitylogicalname eq '${this._targetEntityType}' and meo_fieldlogicalname eq '${fieldName}' and meo_definition eq 'RequiredLevel')`;

    const results = await getFetchResponse(request);

    return results.value[0].meo_definitionvalue;
  }
  ...
  public async isCalculatedField(fieldName: string | undefined) {
    // eslint-disable-next-line max-len
    const request = `/_api/meo_entitydefinitionses?$select=meo_definitionvalue&$filter=(meo_entitylogicalname eq '${this._targetEntityType}' and meo_fieldlogicalname eq '${fieldName}' and meo_definition eq 'IsValidForCreate')`;
    const results = await getFetchResponse(request);

    // eslint-disable-next-line max-len
    if (fieldName === 'meo_measure' || fieldName === 'meo_information' || fieldName === 'meo_tomid') return true;

    return results.value[0].meo_definitionvalue === 'False';
  }
  ...
  Promise<boolean> {
    // eslint-disable-next-line max-len
    const request = `/_api/meo_entitydefinitionses?$select=meo_definitionvalue&$filter=(meo_entitylogicalname eq '${this._targetEntityType}' and meo_fieldlogicalname eq '${columnName}' and meo_definition eq 'IsSecured')`;
    const result = await getFetchResponse(request);
    return result.value[0].meo_definitionvalue === 'True';
  }
